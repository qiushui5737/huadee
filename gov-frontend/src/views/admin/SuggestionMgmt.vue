  ...
  <div class="suggestion-mgmt">
    <el-card shadow="hover">
      ...
      <el-table :data="listData" stripe style="width: 100%">
        <el-table-column prop="suggestNo" label="建议单号" width="180" />
        <el-table-column prop="realName" label="建议人" width="90" />
    ...
    <el-dialog v-model="detailVisible" title="建议详情" width="700px">
      <el-descriptions :column="2" border v-if="currentItem">
        <el-descriptions-item label="建议单号" :span="2">{{ currentItem.suggestNo }}</el-descriptions-item>
        <el-descriptions-item label="建议人">{{ currentItem.realName }}</el-descriptions-item>
        ...
        <el-descriptions-item label="建议类型">{{ currentItem.type }}</el-descriptions-item>
        ...
        <el-descriptions-item label="建议标题" :span="2">{{ currentItem.title }}</el-descriptions-item>
    ...
    <!-- 答复弹窗 -->
    <el-dialog v-model="replyVisible" title="答复建议" width="550px">
      <el-form label-width="80px">
        <el-form-item label="建议单号"><span>{{ currentItem?.suggestNo }}</span></el-form-item>
        <el-form-item label="建议标题"><span>{{ currentItem?.title }}</span></el-form-item>
        <el-form-item label="建议内容"><div class="preview">{{ currentItem?.content }}</div></el-form-item>
    ...
    <!-- 状态/进度编辑弹窗 -->
    <el-dialog v-model="statusEditVisible" title="修改建议进度" width="400px">
      <el-form label-width="80px">
        <el-form-item label="建议单号"><span>{{ statusEditForm.suggestNo }}</span></el-form-item>
...
const statusEditForm = reactive({ suggestNo: '', status: '' })
...
const loadStats = async () => {
  try {
    const res: any = await suggestionStats()
    if (res.code === 200) {
...
const handleReply = async () => {
  ...
  replying.value = true
  try {
    const res: any = await replySuggestion(currentItem.value.suggestNo, {
...
const handleFinish = async (row: any) => {
  try {
    await ElMessageBox.confirm(`确认将建议「${row.suggestNo}」办结？`, '办结确认')
    const res: any = await finishSuggestion(row.suggestNo)
    if (res.code === 200) {
...
const showStatusEdit = (row: any) => {
  statusEditForm.suggestNo = row.suggestNo
...
const handleStatusEdit = async () => {
  ...
  try {
    const res: any = await updateSuggestionStatus(statusEditForm.suggestNo, { status: statusEditForm.status })
    if (res.code === 200) {
