<template>
  <div class="consultation-mgmt">
    <el-card shadow="hover">
      <template #header>
        <div class="card-header">
          <span>咨询管理</span>
          <div class="header-actions">
            <el-input v-model="filterKeyword" placeholder="搜索姓名/单号/主题..." size="small" style="width: 220px; margin-right: 10px;" clearable @clear="loadList" @keyup.enter="loadList">
      ...
      <el-table :data="listData" stripe style="width: 100%">
        <el-table-column prop="consultNo" label="咨询单号" width="180" />
        <el-table-column prop="realName" label="咨询人" width="90" />
        <el-table-column prop="title" label="咨询主题" min-width="200" show-overflow-tooltip />
    ...
    <!-- 详情弹窗 -->
    <el-dialog v-model="detailVisible" title="咨询详情" width="650px">
      <el-descriptions :column="1" border v-if="currentItem">
        <el-descriptions-item label="咨询单号">{{ currentItem.consultNo }}</el-descriptions-item>
        <el-descriptions-item label="咨询人">{{ currentItem.realName }}</el-descriptions-item>
        <el-descriptions-item label="手机号">{{ currentItem.phone || '-' }}</el-descriptions-item>
        <el-descriptions-item label="电话号码">{{ currentItem.telephone || '-' }}</el-descriptions-item>
        <el-descriptions-item label="电子邮箱">{{ currentItem.email || '-' }}</el-descriptions-item>
        <el-descriptions-item label="咨询主题">{{ currentItem.title }}</el-descriptions-item>
        <el-descriptions-item label="咨询市州">{{ currentItem.city || '-' }}</el-descriptions-item>
        <el-descriptions-item label="事件发生地" v-if="currentItem.district || currentItem.street">
          {{ [currentItem.district, currentItem.street, currentItem.town].filter(Boolean).join(' / ') || '-' }}
        </el-descriptions-item>
        <el-descriptions-item label="具体地址" v-if="currentItem.address">{{ currentItem.address }}</el-descriptions-item>
        <el-descriptions-item label="是否公开">{{ currentItem.isPublic ? '是' : '否' }}</el-descriptions-item>
        <el-descriptions-item label="是否保密">{{ currentItem.isConfidential ? '是' : '否' }}</el-descriptions-item>
        ...
        <el-descriptions-item label="咨询内容" style="white-space: pre-wrap;">{{ currentItem.content }}</el-descriptions-item>
        <el-descriptions-item label="附件" v-if="currentItem.attachment">
          <div class="attachment-list">
            <template v-for="(file, idx) in parseAttachments(currentItem.attachment)" :key="idx">
    ...
    <!-- 答复弹窗 -->
    <el-dialog v-model="replyVisible" title="答复咨询" width="550px">
      <el-form label-width="80px">
        <el-form-item label="咨询单号"><span>{{ currentItem?.consultNo }}</span></el-form-item>
        <el-form-item label="咨询主题"><span>{{ currentItem?.title }}</span></el-form-item>
        <el-form-item label="咨询内容"><div class="preview">{{ currentItem?.content }}</div></el-form-item>
    ...
    <!-- 状态编辑弹窗 -->
    <el-dialog v-model="statusEditVisible" title="编辑咨询状态" width="400px">
      <el-form label-width="80px">
        <el-form-item label="咨询单号"><span>{{ statusEditForm.consultNo }}</span></el-form-item>
...
import { consultationList, replyConsultation, updateConsultationStatus, finishConsultation, consultationStats } from '@/api/interaction'
...
const statusEditForm = reactive({ consultNo: '', status: '' })
...
const loadList = async () => {
  try {
    const res: any = await consultationList({
...
const loadStats = async () => {
  try {
    const res: any = await consultationStats()
    if (res.code === 200) {
...
const handleReply = async () => {
  ...
  replying.value = true
  try {
    const res: any = await replyConsultation(currentItem.value.consultNo, {
...
const handleFinish = async (row: any) => {
  try {
    await ElMessageBox.confirm(`确认将咨询「${row.consultNo}」办结？`, '办结确认')
    const res: any = await finishConsultation(row.consultNo)
    if (res.code === 200) {
...
const showStatusEdit = (row: any) => {
  statusEditForm.consultNo = row.consultNo
...
const handleStatusEdit = async () => {
  ...
  try {
    const res: any = await updateConsultationStatus(statusEditForm.consultNo, { status: statusEditForm.status })
    if (res.code === 200) {
...
.consultation-mgmt { padding: 20px; }
